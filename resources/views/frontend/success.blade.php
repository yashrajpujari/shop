    ...
    <div class="col-md-12">
      <div class="card shadow p-3 mb-5 bg-body rounded">
        ...
        <div class="invoice p-5">
          <h5 style="text-align:center;">You order has been confirmed and will be shipped in next 4-5 days!</h5>
          <div class="col-md-12 d-flex">
            @foreach($order_addr as $orderaddress)
            <div class=col-6>

              <span class="font-weight-bold d-block mt-4">
                <h5 style="  text-transform: capitalize;"><i class="fa fa-home mr-2" aria-hidden="true"></i>{{$orderaddress->address_type}}</h5>
                {{$orderaddress->name}}
              </span>
              <span class="font-weight-bold d-block ">
                {{$orderaddress->address}}</span>
              <span class="font-weight-bold">Phone:{{$orderaddress->phone}}</span><br>
              <span class="font-weight-bold">City:{{$orderaddress->city}}</span><br>
              <span class="font-weight-bold">pin code:{{$orderaddress->pincode}}</span><br>
              <span class="font-weight-bold">Country:{{$orderaddress->country}}</span><br>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td width="60%"> <span class="font-weight-bold">₹{{$item->name}}</span>
                    <div class="product-qty"> <span class="d-block" style="color:black;">Quantity:{{$item->qty}}</span>
                    @php
                        $attribute=json_decode($item->custom_option);
                        @endphp
                        @if ($attribute !== null)
                        @foreach ($attribute as $key => $items)
                        <span style="color:black;"><strong>{{ $key }}</strong>: {{ $items }}</span><br>
                        @endforeach
                        @else
                        <span></span>
                        @endif
              ...
              <table class="table table-borderless">
                <tbody class="totals">
                  ...
                  <tr>
                    ...
                    <td>
                      <div class="text-right"> <span class="text-success">-₹{{$order->coupan_discount}}</span> </div>
                    </td>
                  </tr>
        ...
        <div class="d-flex justify-content-center my-4">
          <button class="btn btn-success"><a href="{{route('home')}}" style="color:white;">continue shopping</a></button>
        </div>
