                    ...
                    <form action="{{route('role.update',$role->id)}}"method="Post">
                        ...
                        @method('PUT')
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="text"  name="name" value="{{$role->name}}"class="form-control" id="basic-default-name" placeholder="Role Name">
                                    @if(session('message'))
                            ...
                            @foreach($permissions as $permission)
                                <span class="m-1">
                                <input class="form-check-input" name="permissions[]" type="checkbox" value="{{$permission->id}}" id="defaultCheck3" 
                                @if(
                                in_array($permission->id,$role->permissions->pluck('id')->toArray())) checked @endif
